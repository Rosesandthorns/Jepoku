...
export const DexModeBoard: FC<DexModeBoardProps> = ({ puzzle, checkAnswersAction, mode }) => {
          ...
          <div className="mb-4 flex items-center justify-between">
            <div className="flex items-center gap-2 text-xl font-bold">
             ...
             <Tooltip>
                ...
                <TooltipContent>
                    <p>Guess the type. Clues are Pokédex numbers or entries.</p>
                </TooltipContent>
            </Tooltip>
          </div>
            ...
            <div className="grid grid-cols-[auto,minmax(0,1fr)] items-center gap-2 sm:gap-4">
                ...
                {Array.from({ length: gridSize }).map((_, i) => (
                  <div key={`col-input-${i}`} className="relative">
                    <Select name={`col-${i}`} disabled={state.isCorrect}>
                      ...
                      <SelectContent>
                        {typeCriteria.map((crit) => ( <SelectItem key={crit} value={crit}>{crit}</SelectItem> ))}
                      </SelectContent>
                    </Select>
              ...
              <div className="grid gap-2 sm:gap-4" style={{ gridTemplateColumns: `repeat(${gridSize}, minmax(0, 1fr))` }}>
                {puzzle.grid.flat().map((pokemon, i) => {
                  ...
                  const displayType = puzzle.dexDisplay?.[row][col];

                  return (
                    <div key={pokemon?.id || i} className="aspect-square w-full sm:w-24 md:w-36 lg:w-48 rounded-lg">
                      {pokemon ? (
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <div className="relative h-full w-full overflow-hidden rounded-lg bg-gray-900 shadow-inner p-3 flex flex-col items-center justify-center text-center">
                              {displayType === 'number' ? (
                                <>
                                  <Hash className="h-8 w-8 text-gray-400 mb-2" />
                          ...
                          <TooltipContent>
                             <p className="capitalize">{state.isCorrect ? pokemon.name : '???'}</p>
                          </TooltipContent>
                        </Tooltip>
                      ) : (
